...
from dataset import get_bone_pairs, prepare_npy_collated_one_side_loaders, \
...
class Task_one_side:
    def __init__(self, n_joints, use_top_side_labels: bool) -> None:
        self.use_cuda = torch.cuda.is_available()
        self.device = 'cuda' if self.use_cuda else 'cpu'
        self.n_joints = n_joints
        self.use_top_side = use_top_side_labels
    ...
    def prepare_dataloaders(
        ...
        self.val_loader, \
        self.test_loader \
            = prepare_npy_collated_one_side_loaders(
                root_dir=root_dir,
                pose_style=pose_style,
                use_top_side=self.use_top_side,
    ...
    def seek_network_weights(self, model_info='', serial_no=1):
        ...
        if self.use_top_side:
            model_info = f'_{model_info}_top_side' if model_info != '' else '_top_side'
        else:
            model_info = f'_{model_info}_bottom_side' if model_info != '' else '_bottom_side'
...
if __name__ == '__main__':
    task = Task_one_side(n_joints=17, use_top_side_labels=False)
