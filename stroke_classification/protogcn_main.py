...
from dataset import prepare_npy_collated_loaders, RandomTranslation_batch, \
                    Dataset_npy
from protogcn import ProtoGCN, ContrastiveLoss
...
class Task:
    ...
    def prepare_dataloaders(self, root_dir: Path, pose_style='J_only'):
        self.train_loader, \
        self.val_loader, \
        self.test_loader \
...
if __name__ == '__main__':
    task = Task()
    task.prepare_dataloaders(root_dir=Path('dataset_npy_collated'), pose_style='J_only')
    task.get_network_architecture(model_name='ProtoGCN')
    task.seek_network_weights(model_info='label_smoothing', serial_no=1)
