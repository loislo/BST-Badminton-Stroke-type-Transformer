...
from dataset import get_bone_pairs, prepare_npy_collated_single_pose_loaders, \
...
class Task_single_pose:
    def __init__(self, n_joints=17, opposite_on_purpose=False) -> None:
        self.use_cuda = torch.cuda.is_available()
        self.device = 'cuda' if self.use_cuda else 'cpu'
        self.n_joints = n_joints
        self.opposite_on_purpose = opposite_on_purpose
    ...
    def prepare_dataloaders(
        ...
        self.val_loader, \
        self.test_loader \
            = prepare_npy_collated_single_pose_loaders(
                root_dir=root_dir,
                pose_style=pose_style,
                opposite_on_purpose=self.opposite_on_purpose,
    ...
    def seek_network_weights(self, model_info='', serial_no=1):
        ...
        string = 'single_pose_opposite' if self.opposite_on_purpose else 'single_pose'
        model_info = f'_{model_info}_{string}' if model_info != '' else f'_{string}'
...
if __name__ == '__main__':
    # Notice: The positions here are still the two players.
    task = Task_single_pose(n_joints=17, opposite_on_purpose=True)
